...
#Compare different models, to check whether there is a joint NA/Taiwan effect
#Add a native/invasie factor
for_model$ancestry <- ifelse(for_model$population == "south_america", "native", "invasive")
for_model$continent <- ifelse(for_model$population == "taiwan", "taiwan", "america")

#Run the model with the new factors
lme_ancestry <- lmer(lfc_norm ~ 0 + gene * ancestry + gene * continent  + (1|body_part:gene),
                        data = for_model, weights = weights)

ancestry_aov <- anova(lme_ancestry)
#The gene:ancestry interaction is significant and larger than the gene:continent one. E
#Now look at the list of siginificant genes between NA and Taiwan only
for_model_invasive <- subset(for_model, subset = ancestry == "invasive")

lme_invasive <- lmer(lfc_norm ~ 0 + gene + (1|population:gene),
                     data = for_model_invasive,
                     weights = weights)

# extract coefficients
coefs_inv <- data.frame(coef(summary(lme_invasive)))
#Adjust p values
coefs_inv$padj <- p.adjust(coefs_inv$Pr...t.., method = "BH")
#Significant genes (only those with main gene effects are considered)
sig_genes_inv_lme <- coefs_inv[coefs_inv$padj < 0.05, ]

#Get the names of the significant genes from the model (main effects only)
sig_genes_inv_lme <- rownames(sig_genes_inv_lme)
#Get main effects only
sig_genes_inv_lme <- sig_genes_inv_lme[!grepl(pattern = ":population", x = sig_genes_inv_lme)]
#Clean the names
sig_genes_inv_lme <- gsub(pattern = "gene", replacement = "", x = sig_genes_inv_lme)
...
#Calculate correlation weighted by expression level of each gene
na_sa_corr <- weightedCorr(lfcs_na$lfc_norm, lfcs_sa$lfc_norm, weights = log(mean_expr_gene$expr),
                           method = "spearman")
#pdf("results/na_sa_lfcs.pdf", width = 10, height = 7.9)
jpeg("results/na_sa_lfcs.jpg", width = 10, height = 7.9,
     res = 300, units = "in")
par(mgp = c(2.2, 0.5, 0), cex = 2)
plot(lfcs_na$lfc_norm, lfcs_sa$lfc_norm, col = colors_plot, cex = log(mean_expr_gene$expr),
     xlab = "SB/Sb log2 expression ratios\nNorth America", ylab = "SB/Sb log2 expression ratios\nSouth America",
     main = paste("r² =", round(na_sa_corr, digits = 2)))
abline(h = 0, col = "red")
abline(v = 0, col = "red")
dev.off()

na_tai_corr <- weightedCorr(lfcs_na$lfc_norm, lfcs_taiwan$lfc_norm, weights = log(mean_expr_gene$expr),
                            method = "spearman")
#pdf("results/na_tai_lfcs.pdf", width = 10, height = 7.9)
jpeg("results/na_tai_lfcs.jpg", width = 10, height = 7.9,
     res = 300, units = "in")
par(mgp = c(2.2, 0.5, 0), cex = 2)
plot(lfcs_na$lfc_norm, lfcs_taiwan$lfc_norm, col = colors_plot, cex = log(mean_expr_gene$expr),
     xlab = "SB/Sb log2 expression ratios\nNorth America", ylab = "SB/Sb log2 expression ratios\nTaiwan",
     main = paste("r² =", round(na_tai_corr, digits = 2)))
abline(h = 0, col = "red")
abline(v = 0, col = "red")
dev.off()

sa_tai_corr <- weightedCorr(lfcs_sa$lfc_norm, lfcs_taiwan$lfc_norm, weights = log(mean_expr_gene$expr),
                            method = "spearman")
#pdf("results/sa_tai_lfcs.pdf", width = 10, height = 7.9)
jpeg("results/sa_tai_lfcs.jpg", width = 10, height = 7.9,
     res = 300, units = "in")
par(mgp = c(2.2, 0.5, 0), cex = 2)  
plot(lfcs_sa$lfc_norm, lfcs_taiwan$lfc_norm, col = colors_plot, cex = log(mean_expr_gene$expr),
     xlab = "SB/Sb log2 expression ratios\nSouth America", ylab = "SB/Sb log2 expression ratios\nTaiwan",
     main = paste("r² =", round(sa_tai_corr, digits = 2)))
abline(h = 0, col = "red")
abline(v = 0, col = "red")
dev.off()
...
jpeg("results/genotype_social_form_corr.jpg", width = 10, height = 7.9,
     res = 300, units = "in")
...
sig_genes_taiwan <- rownames(results_taiwan_subset[which(results_taiwan_subset$padj < 0.05), ])
...
facet_labels <- c(QA = "South America\nQueen Abdomen", QH = "South America\nQueen Head", 
                  ...
                  whole_body_queen = "North America\nQueen Whole", whole_body_queen_taiwan = "Taiwan\nQueen Whole")
...
#Select genes with significant DE in the different populations and in both
...
genes_common_char <- as.character(parsed_counts_all_ratios_common_genes$gene)
pops_char <- as.character(parsed_counts_all_ratios_common_genes$population)

parsed_counts_all_ratios_common_genes$sig[genes_common_char %in% sig_genes_na &
                                            pops_char == "north_america"] <- "na"

parsed_counts_all_ratios_common_genes$sig[genes_common_char %in% sig_genes_sa &
                                            pops_char == "south_america"] <- "sa"

parsed_counts_all_ratios_common_genes$sig[genes_common_char %in% sig_genes_taiwan &
                                            pops_char == "taiwan"] <- "taiwan"


parsed_counts_all_ratios_common_genes$sig[genes_common_char %in% sig_genes_na &
                                            genes_common_char %in% sig_genes_sa &
                                            (pops_char == "north_america" | 
                                            pops_char == "south_america")] <- "america" 

parsed_counts_all_ratios_common_genes$sig[genes_common_char %in% sig_genes_na &
                                            genes_common_char %in% sig_genes_taiwan &
                                            (pops_char == "north_america" | 
                                            pops_char == "taiwan")] <- "invasive"

parsed_counts_all_ratios_common_genes$sig[genes_common_char %in% sig_genes_sa &
                                            genes_common_char %in% sig_genes_taiwan &
                                            (pops_char == "south_america" | 
                                            pops_char == "taiwan")] <- "sa_taiwan"

parsed_counts_all_ratios_common_genes$sig[genes_common_char %in% sig_genes_both_lme] <- "all"
...
pos_sig_genes <- as.character(
  unique(parsed_counts_all_ratios_common_genes$gene[parsed_counts_all_ratios_common_genes$sig != "no"]))
...
p3 <- ggplot(data = parsed_counts_all_ratios_common_genes, aes(x = gene, y = lfc)) + geom_boxplot(varwidth = TRUE, aes(weight = log(medians + 1.1), 
        ...
        scale_fill_manual(values=c(hcl(15,100,75), "#999999", hcl(195,100,75), hcl(80,100,75))) +
        scale_color_manual(values=c(hcl(15,100,50), "grey40", hcl(195,100,50), hcl(80,100,50))) +
        facet_grid(rows = vars(body_part), scales = "free", labeller = labeller(body_part = facet_labels)) +
...
parsed_counts_north_america_ratio$sig[parsed_counts_north_america_ratio$gene %in% sig_genes_na_all] <- "na"

#Generate x axis only with significant genes
x_axis_genes <- rep("", length(levels(parsed_counts_north_america_ratio$gene)))

pos_sig_genes <- which(levels(parsed_counts_north_america_ratio$gene) %in% sig_genes_na_all)
pos_sig_genes <- sort(pos_sig_genes)
...
Run the same format for the Taiwan only data
```{r boxplot_ratios_north_taiwan_format2, message = FALSE, warning = FALSE}

#Calculate median read counts per group (gene and body part)
agg_medians_taiwan <- aggregate(total_counts ~ gene + body_part, data = parsed_counts_taiwan_ratio, median)
agg_medians_taiwan <- agg_medians_taiwan[order(agg_medians_taiwan$gene, agg_medians_taiwan$body_part), ]
colnames(agg_medians_taiwan) <- c("gene", "body_part", "medians")

#Add medians to the main dataset
parsed_counts_taiwan_ratio <- merge(agg_medians_taiwan, parsed_counts_taiwan_ratio)

#Get only the minimum start position in the scaffolds for all the transcripts per gene
gene_positions_min_taiwan <- aggregate(x = gene_positions$start, by = list(gene_positions$contig, gene_positions$gene), FUN = min)
names(gene_positions_min_taiwan) <- c("contig", "gene", "start_position")

#Select genes only in the analysis
gene_positions_min_taiwan <- gene_positions_min_taiwan[gene_positions_min_taiwan$gene %in%
                                                         unique(parsed_counts_taiwan_ratio$gene), ]

#Sort by start position per contig
gene_positions_min_taiwan <- gene_positions_min_taiwan[order(gene_positions_min_taiwan$contig, gene_positions_min_taiwan$start_position), ]

#Get a list of all the significant genes in the supergene for Taiwan
sig_genes_taiwan_all <- rownames(results_taiwan[which(results_taiwan$padj < 0.05), ])

#Add a column specifying which genes are significant 
parsed_counts_taiwan_ratio$sig <- rep ("no", nrow(parsed_counts_taiwan_ratio))
parsed_counts_taiwan_ratio$sig[parsed_counts_taiwan_ratio$gene %in% sig_genes_taiwan_all] <- "taiwan"

#Generate x axis only with significant genes
x_axis_genes <- rep("", length(levels(parsed_counts_taiwan_ratio$gene)))

pos_sig_genes <- which(levels(parsed_counts_taiwan_ratio$gene) %in% sig_genes_taiwan_all)
pos_sig_genes <- sort(pos_sig_genes)
x_axis_genes_ids <- levels(parsed_counts_taiwan_ratio$gene)[pos_sig_genes]
x_axis_genes[pos_sig_genes] <- x_axis_genes_ids

#Plot the thing
p3b <- ggplot(data = parsed_counts_taiwan_ratio, aes(x = gene, y = lfc)) + geom_boxplot(varwidth = TRUE, aes(weight = log(medians + 1.1), 
                                                                                                                 fill = sig, color = sig), outlier.size = 0.5) +
        scale_fill_manual(values=c("#999999", "darkolivegreen3")) +
        scale_color_manual(values=c("grey40", "chartreuse3")) +
        geom_hline(yintercept = 0, color = "black") + geom_hline(yintercept = -1, color = "darkgrey", linetype = "dashed") + 
...
#Save the plot
#ggsave(filename = "results/boxplot_lfcs_taiwan_format2.pdf", plot = p3b, device = "pdf", height = 210, width = 297, units = "mm")
...
#Select only significant genes for the genes in common between SA, NA and Taiwan
sig_genes_plot <- unique(c(sig_genes_both_lme, sig_genes_sa, sig_genes_na, sig_genes_taiwan))
parsed_counts_sig_ratios_common_genes <- subset(x = parsed_counts_all_ratios_common_genes, 
                                                subset = parsed_counts_all_ratios_common_genes$gene %in% sig_genes_plot)
...
Boxplot with significant genes in SA, NA and Taiwan, comparing lfcs per gene across populations/body parts
...
p5 <- ggplot(data = parsed_counts_sig_ratios_common_genes, aes(x = body_part, y = lfc)) + geom_boxplot(varwidth = TRUE, aes(weight = log(medians + 1.1), 
                                                                                                                 fill = sig, color = sig), outlier.size = 0.5) +
               scale_fill_manual(values=c(hcl(15,100,75), "#999999", hcl(195,100,75), hcl(139, 255, 98)),
                          breaks = c("all", "sa", "sa_taiwan", "no"), 
                          labels = c("All populations", "South America\nonly", "Taiwan and\nSouth America",
                                     "None"),
                          name = "Significant differences\nbetween variants") +
        scale_color_manual(values=c(hcl(15,100,50), "grey40", hcl(195,100,50), hcl(139, 255, 63)),
                          c("all", "sa", "sa_taiwan", "no"), 
                          labels = c("All populations", "South America\nonly", "Taiwan and\nSouth America",
                                     "None"),
                          name = "Significant differences\nbetween variants") +
        facet_grid(. ~ gene) +
...
fill_colors <- c(rgb(152,78,163, maxColorValue = 255), rgb(55,126,184, maxColorValue = 255),
                 ...
                 rgb(0,255,243, maxColorValue = 255), rgb(247,255,0, maxColorValue = 255))
contour_colors <- c(rgb(106, 55, 114, maxColorValue = 255), rgb(39, 88, 129, maxColorValue = 255),
                 ...
                 rgb(0,142,135, maxColorValue = 255), rgb(175,181,0, maxColorValue = 255))
...
body_part_factor <- factor(parsed_counts_sig_ratios_common_genes$body_part,
                           levels = c("W", "QH", "QT", "QA", "whole_body_queen",  "whole_body_queen_taiwan"))
...
p8 <- ggplot(data = parsed_counts_sig_ratios_common_genes, aes(x = body_part, y = lfc)) + geom_boxplot(varwidth = TRUE, aes(weight = log(medians + 1), 
                                                                                                       fill = body_part, color = body_part), outlier.size = 0.5) +
        scale_fill_manual(values = fill_colors,
...
euler_sig_genes <- euler(combinations = list("South America only" = sig_genes_sa,
                                    ...
                                    "Taiwan only" = sig_genes_taiwan_all,
