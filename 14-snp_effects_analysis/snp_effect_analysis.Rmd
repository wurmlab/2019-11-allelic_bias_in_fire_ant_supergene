...
#Remove extra row for the two transcripts of the gene LOC105199327
snp_effect_candidates$TranscriptId <- NULL
snp_effect_candidates <- snp_effect_candidates[!duplicated(snp_effect_candidates), ]
...
snp_effect_impact_parsed <- data.frame(gene_id,
...
snp_effect_candidates_position_parsed <- data.frame(gene_type,
                                                    nb_snps = c(as.matrix(snp_effect[, position_names])),
                                                    position)

plot_position <- ggplot(data = snp_effect_candidates_position_parsed, aes(y = log(nb_snps + 1), x = gene_type, fill = position)) + geom_boxplot() +
                                  theme_bw() 
